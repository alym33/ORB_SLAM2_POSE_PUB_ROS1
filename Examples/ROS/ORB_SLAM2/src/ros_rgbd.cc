...
//Ali
   igb.pub_pc_ = pnh.advertise<sensor_msgs::PointCloud>("pointcloud", 1);
    igb.pub_pose_ = pnh.advertise<geometry_msgs::PoseStamped>("pose", 1);
    igb.pub_pose2d_ = pnh.advertise<geometry_msgs::PoseStamped>("pose2d", 1);
    ros::spin();
...
void ImageGrabber::GrabRGBD(const sensor_msgs::ImageConstPtr& msgRGB,const sensor_msgs::ImageConstPtr& msgD)
{
    ...
    cv::Mat Tcw = mpSLAM->TrackRGBD(cv_ptrRGB->image,cv_ptrD->image,cv_ptrRGB->header.stamp.toSec()); //Ali
    publishPose(Tcw, cv_ptrRGB->header); //Ali
}
